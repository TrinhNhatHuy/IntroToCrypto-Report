...
% ── Stage 2: Hashing ──
\node[box, fill=orange!12, below=0.8cm of keygen] (msg) {Message $m$};
\node[box, fill=orange!12, below=of msg] (hash) {Hash: $H = \texttt{hash}(m)$};
\draw[arrow] (msg) -- (hash);

% ── Stage 3: Signing — Private Key on the SAME ROW ──
...
% ── Stage 4: Verification — Public Key on the SAME ROW ──
...
% Key Gen outputs (short dashed to labels only)
\draw[arrow, dashed] (keygen.west) -| (privkey.north);
\draw[arrow, dashed] (keygen.east) -| (pubkey.north);

% Result
\node[font=\small\bfseries, color=green!50!black, below=0.25cm of compare] {Valid / Invalid};

\end{tikzpicture}
\caption{RSA Digital Signature: Key Generation $\rightarrow$ Signing $\rightarrow$ Verification.}
\label{fig:flowchart}
\end{figure}
